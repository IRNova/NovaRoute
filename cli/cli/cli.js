...
// Subcommands (`NovaRoute xai video …`) run against an already-running gateway
...
// Self-heal SQLite runtime deps (sql.js + better-sqlite3) into ~/.novaroute/runtime
...
// Identifiers for killAllAppProcesses - only kill NovaRoute specifically
const PROCESS_IDENTIFIERS = [
  'NovaRoute'  // Only package name - avoid killing other apps
];
...
function getAppDataDir() {
  return process.platform === "win32"
    ? path.join(process.env.APPDATA || "", "NovaRoute")
    : path.join(os.homedir(), ".novaroute");
}
    ...
    try {
      ...
      if (platform === "win32") {
        ...
        try {
          ...
          const lines = output.split("\n").slice(1).filter(l => l.trim());
          lines.forEach(line => {
            // Whitelist: real node process running NovaRoute/cli.js, or next-server.
            // Avoids killing editors/grep/strace/cursor that just have "NovaRoute" in cmdline.
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("NovaRoute") && (cmd.includes("cli.js") || cmd.includes("\\NovaRoute") || cmd.includes("/NovaRoute")))
        ...
        try {
          ...
          lines.forEach(line => {
            // Whitelist: real node process running NovaRoute/cli.js, or next-server.
            // Avoids killing grep/strace/editors/cursor that incidentally match "NovaRoute".
            const cmd = line.toLowerCase();
            const isAppProcess =
              (cmd.includes("node") && cmd.includes("NovaRoute") && (cmd.includes("cli.js") || cmd.includes("/NovaRoute")))
              || cmd.includes("next-server");
            if (isAppProcess) {
              const parts = line.trim().split(/\s+/);
              const pid = parts[1];
...
//
// Off unless an IRNova-owned npm package is configured. This fork inherited the
// upstream author's package name, so an unguarded check would report their
// releases as updates to this build and offer to install them.
const UPDATE_PACKAGE = process.env.NOVAROUTE_NPM_PACKAGE || "";
const UPDATE_CHANNEL_ON =
  process.env.NOVAROUTE_UPDATE_CHANNEL === "npm" && UPDATE_PACKAGE !== "";

function checkForUpdate() {
  return new Promise((resolve) => {
    if (skipUpdate || !UPDATE_CHANNEL_ON) {
      resolve(null);
      return;
    }
    ...
    const req = https.get(`https://registry.npmjs.org/${encodeURIComponent(UPDATE_PACKAGE)}/latest`, { timeout: 3000 }, (res) => {
      let data = "";
...
function startServer(updatePromise) {
      ...
      while (true) {
        ...
        if (choice === "update") {
          ...
          if (process.platform === "darwin") {
            ...
            console.log(`🔔 NovaRoute is running in tray (PID: ${process.pid})`);
            console.log(`   Server: http://${displayHost}:${port}`);
          ...
          console.log(`🔔 NovaRoute is now running in background (PID: ${bgProcess.pid})`);
          console.log(`   Server: http://${displayHost}:${port}`);
          console.log(`\n💡 You can close this terminal. Right-click tray icon to quit.\n`);
  ...
  function tryRestart(code) {
    ...
    if (restartCount >= MAX_RESTARTS) {
      ...
      try {
        const dbPath = path.join(os.homedir(), process.platform === "win32" ? path.join("AppData", "Roaming", "NovaRoute", "db.json") : path.join(".novaroute", "db.json"));
