...
async function buildCodexHeader() {
  ...
  const { installed, hasNovaRoute, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, hasNovaRoute, settings } = result.data;
  ...
  // Extract NovaRoute custom model config
  const custom = settings?.customModels?.find(m => m.id === "custom:NovaRoute-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  // Extract NovaRoute provider config
  const provider = settings?.models?.providers?.["NovaRoute"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("NovaRoute/") ? primary.replace("NovaRoute/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, hasNovaRoute, opencode } = result.data;
...
async function showCliToolsMenu(port, breadcrumb = []) {
  ...
  await showMenuWithBack({
    title: "🔧 CLI Tools",
    breadcrumb,
    headerContent: `Configure CLI tools to use NovaRoute\nEndpoint: ${endpoint}`,
    items: [
      {
        label: "Claude Code",
