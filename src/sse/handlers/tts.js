import {
  extractApiKey, isValidApiKey, isInternalCliRequest,
...
async function handleTtsImpl(request) {
  let body;
  try {
  ...
  {
    const policy = internalCli ? { rejection: null } : await enforceApiKeyPolicy(request, comboModels ? null : modelStr);
...
export const handleTts = tracedHandler("audio.speech", handleTtsImpl);
