 ...
 */
async function handleEmbeddingsImpl(request) {
  let body;
  try {
  ...
  // Enforce API key if enabled in settings (internal self-tests bypass)
  ...
  {
    const policy = internalCli ? { rejection: null } : await enforceApiKeyPolicy(request, modelStr);
...
export const handleEmbeddings = tracedHandler("embeddings", handleEmbeddingsImpl);
