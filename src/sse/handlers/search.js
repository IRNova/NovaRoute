...
import { isInternalCliRequest } from "../services/auth.js";
 ...
 */
async function handleSearchImpl(request) {
  let body;
  try {
  ...
  if (!(await isInternalCliRequest(request))) {
    const policy = await enforceApiKeyPolicy(request, comboModels ? null : providerInput);
  ...
  if (comboModels) {
    ...
    return handleComboChat({
      ...
      comboStrategy,
      comboStickyLimit,
      comboStrategyConfig: comboStrategies[providerInput] || {},
    });
  }
...
export const handleSearch = tracedHandler("search", handleSearchImpl);
