...
function withConnectionHeader(response, connectionId) {
  ...
  headers.set("x-NovaRoute-connection-id", String(connectionId));
 ...
 */
async function handleVideoCreateImpl(request, action) {
  ...
  // Per-key policy (active flag, model/group access, USD spend limits) applies to
  // every billable surface, not just chat: a key capped at $1/day must not be able
  // to spend it on images, video or speech instead.
  if (!(await isInternalCliRequest(request))) {
    const policy = await enforceApiKeyPolicy(request, model, provider);
 ...
 */
async function handleVideoGetImpl(request, requestId) {
  const authError = await requireValidApiKey(request);
  if (authError) return authError;
...
export const handleVideoCreate = tracedHandler("videos.create", handleVideoCreateImpl);

// Traced at the boundary: one span per gateway call, with the provider,
// model and token counts filled in by saveRequestUsage. No-op unless an
// OTLP endpoint is configured.
export const handleVideoGet = tracedHandler("videos.get", handleVideoGetImpl);
