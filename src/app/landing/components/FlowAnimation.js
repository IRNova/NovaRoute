...
export default function FlowAnimation() {
  ...
  return (
    <div className="mt-16 w-full max-w-4xl relative h-[360px] hidden md:flex items-center justify-center animate-[float_6s_ease-in-out_infinite]">
      {/* NovaRoute Hub - Center */}
      <div className="relative z-20 w-32 h-32 rounded-full bg-elevated border-2 border-[color-mix(in_srgb,var(--cyan)_70%,transparent)] shadow-[var(--shadow-accent)] flex flex-col items-center justify-center gap-1 group cursor-pointer hover:scale-105 transition-transform duration-500 overflow-hidden">
        <img src="/logo-mark-mono.svg" alt="" aria-hidden="true" className="size-12 object-contain" />
        <span className="text-xs font-bold text-text-main tracking-widest uppercase">
          NovaRoute
        </span>
        <div className="absolute inset-0 rounded-full border border-[color-mix(in_srgb,var(--cyan)_30%,transparent)] animate-ping opacity-20"></div>
      </div>
      ...
      <div className="absolute left-0 top-1/2 -translate-y-1/2 flex flex-col gap-7">
        {CLI_TOOLS.map((tool) => (
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-surface border border-border flex items-center justify-center overflow-hidden p-2 hover:border-[color-mix(in_srgb,var(--cyan)_50%,transparent)] transition-all hover:scale-105">
              <ProviderIcon
                src={tool.image}
                alt={tool.name}
                size={48}
      ...
      {/* SVG Lines from CLI to NovaRoute */}
      <svg
        className="absolute inset-0 w-full h-full z-10 pointer-events-none stroke-[color:var(--border-strong)]"
      ...
      {/* SVG Lines from NovaRoute to Providers */}
      <svg
      ...
      >
        <path
          ...
          fill="none"
          stroke={activeFlow === 0 ? "var(--cyan)" : "var(--border-strong)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 1 ? "var(--cyan)" : "var(--border-strong)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 2 ? "var(--cyan)" : "var(--border-strong)"}
        ...
        <path
          ...
          fill="none"
          stroke={activeFlow === 3 ? "var(--cyan)" : "var(--border-strong)"}
        ...
        {PROVIDERS.map((provider, idx) => (
          <div
            ...
            className={`px-4 py-2 rounded-lg ${provider.color} ${provider.textColor} flex items-center justify-center font-bold text-xs shadow-lg hover:scale-110 transition-all cursor-help min-w-[140px] ${
              activeFlow === idx ? "ring-4 ring-[color-mix(in_srgb,var(--cyan)_50%,transparent)] scale-110" : ""
            }`}
            title={provider.name}
          >
            {provider.name}
