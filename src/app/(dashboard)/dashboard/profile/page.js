...
const SETTINGS_TABS = [
  ...
  { id: "update", label: translate("Update"), icon: "system_update_alt" },
  { id: "security", label: translate("Security"), icon: "shield" },
  { id: "routing", label: translate("Routing"), icon: "route" },
  { id: "quota", label: translate("Quota"), icon: "data_usage" },
  { id: "network", label: translate("Network"), icon: "wifi" },
  { id: "optimization", label: translate("Optimization"), icon: "auto_awesome" },
  { id: "observability", label: translate("Observability"), icon: "monitoring" },
  { id: "danger", label: translate("Danger Zone"), icon: "warning" },
];

// Grouped navigation for the settings rail.
const TAB_GROUPS = [
  { id: "system", label: translate("System"), tabs: ["general", "update"] },
  { id: "access", label: translate("Access"), tabs: ["security"] },
  { id: "performance", label: translate("Performance"), tabs: ["routing", "quota", "optimization"] },
  { id: "monitoring", label: translate("Monitoring"), tabs: ["network", "observability"] },
  { id: "advanced", label: translate("Advanced"), tabs: ["danger"] },
];
...
const TAB_DESCRIPTIONS = {
  general: "Theme, language and general gateway behaviour.",
  update: "Check GitHub releases and auto-update the gateway.",
  security: "Login, password, OIDC sign-in and database tools.",
  routing: "Fallback strategy, combo ordering and smart routing.",
  quota: "Automatic provider quota polling.",
  network: "Router endpoint and outbound proxy forwarding.",
  optimization: "Semantic cache, compression and prompt optimizer.",
  observability: "Usage tracking and log retention tuning.",
  danger: "Remotely stop the gateway process.",
};
...
export default function ProfilePage() {
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `NovaRoute-backup-${stamp}.json`;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-0">
      {/* Aurora hero header */}
      <div className="relative overflow-hidden rounded-3xl border border-border/80 bg-surface mb-6 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.25)]">
        <div aria-hidden className="absolute inset-0 bg-gradient-to-br from-primary/[0.13] via-transparent to-success/[0.08]" />
        <div aria-hidden className="absolute -top-28 -right-10 h-64 w-64 rounded-full bg-primary/20 blur-[80px]" />
        <div aria-hidden className="absolute -bottom-24 left-1/4 h-52 w-52 rounded-full bg-violet-500/15 blur-[70px]" />
        <div
          aria-hidden
          className="absolute inset-0 opacity-[0.35]"
          style={{
            backgroundImage:
              "linear-gradient(to right, color-mix(in srgb, var(--bd) 55%, transparent) 1px, transparent 1px), linear-gradient(to bottom, color-mix(in srgb, var(--bd) 45%, transparent) 1px, transparent 1px)",
            backgroundSize: "34px 34px",
            maskImage: "radial-gradient(ellipse at top right, black 20%, transparent 72%)",
            WebkitMaskImage: "radial-gradient(ellipse at top right, black 20%, transparent 72%)",
          }}
        />
        <div className="relative p-6 sm:p-7 flex flex-wrap items-center gap-4">
          <div className="size-14 rounded-2xl bg-surface/70 backdrop-blur border border-border text-primary flex items-center justify-center shrink-0 shadow-lg shadow-primary/10">
            <span className="material-symbols-outlined text-[30px]">tune</span>
          </div>
          <div className="min-w-0">
            <h1 className="text-[26px] leading-tight font-extrabold tracking-tight text-text-main">{translate("Settings")}</h1>
            <p className="text-sm text-text-muted mt-0.5">{translate("Manage your gateway preferences, security and updates.")}</p>
          </div>
          <div className="ms-auto flex items-center gap-2 shrink-0">
            ...
            <Badge variant="primary" size="md" className="gap-1 font-mono">
      ...
      <div className="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <nav
          ...
          className="w-full lg:w-60 lg:sticky lg:top-6 shrink-0"
        >
          <div className="lg:rounded-2xl lg:border lg:border-border lg:bg-surface/70 lg:backdrop-blur-xl lg:p-2 lg:shadow-sm">
            <div className="flex lg:flex-col gap-1 overflow-x-auto pb-1.5 lg:pb-0 -mx-4 px-4 lg:mx-0 lg:px-0 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
              {TAB_GROUPS.map((group) => (
                <div key={group.id} className="contents lg:block">
                  <p className="hidden lg:block px-3 pt-3.5 pb-1 text-[10px] font-bold uppercase tracking-[0.14em] text-text-muted/70 first:pt-1">
                    {group.label}
                  </p>
                  {group.tabs.map((tabId) => {
                    const tab = getTab(tabId);
                    const active = activeTab === tabId;
                    const isDanger = tabId === "danger";
                    return (
                      <button
                        key={tabId}
                        type="button"
                        ...
                        title={tab.label}
                        className={cn(
                          "group relative flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-all duration-150 shrink-0 outline-none focus-visible:ring-2 focus-visible:ring-primary/40",
                          active
                            ? isDanger
                      ...
                      >
                          ...
                          className={cn(
                            ...
                            active
                              ? isDanger
                                ? "bg-red-500/15"
                                : "bg-primary/15"
                              : "bg-surface-2 group-hover:bg-border/60"
                          )}
                        >
                        ...
                        {active && (
                          <span
                            aria-hidden
                            className={cn(
                              "ms-auto hidden lg:block h-1.5 w-1.5 rounded-full",
                              isDanger ? "bg-red-500" : "bg-gradient-to-r from-primary to-success"
                            )}
                          />
                        )}
                      </button>
        ...
        {/* Content column */}
        <div key={activeTab} className="flex-1 min-w-0 w-full flex flex-col gap-5 animate-in fade-in slide-in-from-bottom-1 duration-300">
          {activeTab !== "danger" && (
            <div className="flex items-start gap-3 px-1">
              <div className="mt-0.5 flex size-9 items-center justify-center rounded-xl border border-border bg-surface text-primary shrink-0">
                <span className="material-symbols-outlined text-[19px]">{getTab(activeTab)?.icon}</span>
              </div>
              <div>
                <h2 className="text-base font-bold text-text-main leading-snug">{getTab(activeTab)?.label}</h2>
                <p className="text-xs text-text-muted mt-0.5">{translate(TAB_DESCRIPTIONS[activeTab] || "")}</p>
              </div>
            </div>
          )}

        {activeTab === "update" && <AutoUpdateSection />}
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">~/.novaroute/db/data.sqlite</p>
              </div>
            </div>
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            <div className="size-10 rounded-lg bg-info/10 text-blue-500 flex items-center justify-center shrink-0">
        ...
        <Card>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-start sm:items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm sm:text-base">{translate("Require API key")}</p>
                <p className="text-xs sm:text-sm text-text-muted">
                  When ON, the /v1 gateway rejects requests without a valid API key.
                </p>
              </div>
              <Toggle
            ...
            {settings.requireLogin === true && (
              <form onSubmit={handlePasswordChange} className="flex flex-col gap-4 pt-4 border-t border-border/50">
                ...
                {/* {!settings.hasPassword && (
                  <div className="p-3 rounded-lg bg-info/10 border border-blue-500/20">
          ...
          <div className="flex flex-col gap-4 mt-4">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <Input
                  placeholder="https://auth.example.com/application/o/NovaRoute/"
              ...
              <div className="flex flex-col gap-2">
                ...
                <Input
                  placeholder="NovaRoute-dashboard"
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            <div className="p-2 rounded-lg bg-info/10 text-blue-500 shrink-0">
        ...
        <div className="text-center text-xs sm:text-sm text-text-muted py-4">
          <p>{APP_CONFIG.name} {APP_CONFIG.versionLabel}</p>
