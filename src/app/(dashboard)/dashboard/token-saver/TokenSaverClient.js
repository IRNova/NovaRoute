            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {pendingExtras.length > 0 && (
                <button
                  ...
                  className="text-xs px-2.5 py-1 rounded bg-primary text-on-primary hover:opacity-90 disabled:opacity-50"
                >
                  {extrasActionLoading
                    ? "Installing…"
          ...
          {!pxpipeStatus.installed ? (
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs text-text-muted">
                ...
                the NovaRoute data directory. May take a few minutes.
              </p>
            </div>
          ) : (
...
// ─── Savings ledger card (real RTK compression accounting) ────────────────
function RtkSavingsCard() {
  ...
  if (!stats || !stats.allTime || stats.allTime.requests === 0) return null;
  const { allTime, today } = stats;

  const tiles = [
    { label: "Saved today", value: `${(today.tokensSavedEst || 0).toLocaleString()} tok ≈ $${(today.usdSavedEst || 0).toFixed(3)}` },
    { label: "Saved all-time", value: `${(allTime.tokensSavedEst || 0).toLocaleString()} tok ≈ $${(allTime.usdSavedEst || 0).toFixed(2)}` },
    { label: "Compression ratio", value: `${allTime.savedPct || 0}%` },
    { label: "Compressed requests", value: `${(allTime.compressedRequests || 0).toLocaleString()} / ${(allTime.requests || 0).toLocaleString()}` },
  ];

  return (
    <Card>
      <div className="flex items-center gap-2 mb-3">
        <span className="material-symbols-outlined text-[20px] text-success">savings</span>
        <h2 className="text-base font-semibold text-text-main">Savings</h2>
        <span className="text-xs text-text-muted">
          (est. ~4 chars/token @ ${stats.estPricePerMTok}/1M)
        </span>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {tiles.map((t) => (
          <div key={t.label} className="p-3 rounded-xl bg-surface-3/50 border border-border-subtle">
            <p className="text-xs text-text-muted uppercase tracking-wide">{t.label}</p>
            <p className="text-sm font-bold text-text-main mt-1" dir="ltr">{t.value}</p>
          </div>
        ))}
      </div>
    </Card>
