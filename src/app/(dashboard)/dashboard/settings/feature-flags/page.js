...
export default function FeatureFlagsSettingsPage() {
  ...
  const filtered = FEATURE_FLAGS.filter((f) => {
    ...
    if (search) {
      ...
      if (!f.label.toLowerCase().includes(q) && !f.description.toLowerCase().includes(q)) return false;
    }
    return true;
  });
  ...
  return (
    <div className="space-y-6">
        ...
        return (
          <Card key={cat} className="p-5 space-y-1">
            ...
            {items.map((flag) => (
              <div
                key={flag.key}
                className="flex items-center justify-between gap-4 py-2.5 border-b border-surface-3 last:border-0"
              >
      ...
      <div className="flex justify-end">
        <Button size="sm" variant="outline" onClick={() => save({ featureFlags: {} })}>
