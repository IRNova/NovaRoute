...
export default function CompatibleModelsSection({ providerStorageAlias, providerDisplayAlias, modelAliases, customModels, copied, onCopy, onDeleteAlias, onAddCustomModel, onDeleteCustomModel, connections, isAnthropic }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/models/test-results?providerAlias=${encodeURIComponent(providerStorageAlias)}`, { cache: "no-store" })
      .then((res) => res.json())
      .then((data) => {
        if (data.results) {
          const mapped = {};
          Object.entries(data.results).forEach(([id, record]) => {
            mapped[id] = record.status || record;
          });
          setModelTestResults(mapped);
        }
      })
      .catch((error) => console.log("Error loading compatible model test results:", error));
