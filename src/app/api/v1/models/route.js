...
async function fetchRegistryModels(connection) {
  ...
  try {
    ...
    const response = await fetch(fetcher.url, { method: "GET", headers, cache: "no-store", signal: controller.signal });
...
// and break recursive loops between NovaRoute instances connected to each other.
...
async function fetchCompatibleModelIds(connection) {
  ...
  if (isOpenAICompatibleProvider(connection.provider)) {
    ...
    if (url.endsWith("/messages/models")) {
      url = url.replace(/\/messages\/models$/, "/models");
    }
...
export async function buildModelsList(kindFilter, options = {}) {
  ...
  // NovaRoute instance's fetchCompatibleModelIds — skip dynamic fetch to break
  // cross-instance recursive loops.
  const skipDynamicFetch = options.skipDynamicFetch === true;
  let connections = [];
  try {
    connections = await getProviderConnections();
    // Keep active providers visible even if a previous connection-level probe is stale/failed;
    // per-model disabled state below is the source of truth for hiding bad models.
  ...
  if (connections.length === 0) {
    ...
    for (const [providerId, conn] of activeConnectionByProvider.entries()) {
      ...
      if (!hasExplicitEnabledModels && !skipDynamicFetch) {
        const registryLive = await fetchRegistryModels(conn);
        if (registryLive?.models?.length) {
          rawModelIds = registryLive.models.map((m) => m.id);
          liveCapabilitiesById = new Map(
            registryLive.models
              .filter((m) => m?.id && (m.contextLength || m.maxOutputTokens))
              .map((m) => [m.id, {
                ...(m.contextLength ? { contextWindow: m.contextLength } : {}),
                ...(m.maxOutputTokens ? { maxOutput: m.maxOutputTokens } : {}),
              }])
          );
        }
      }
...
export async function GET(request) {
  try {
    // Detect cross-instance recursive /models fetch (another NovaRoute fetching our /models)
