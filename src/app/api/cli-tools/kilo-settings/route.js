...
const hasNovaRouteConfig = (auth) => {
  if (!auth) return false;
  const entry = auth["openai-compatible"] || auth["NovaRoute"];
  if (!entry) return false;
  const baseUrl = entry.baseUrl || entry.baseURL || "";
  return baseUrl.includes("localhost") || baseUrl.includes("127.0.0.1") || baseUrl.includes("NovaRoute");
};
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasNovaRoute: hasNovaRouteConfig(auth),
...
export async function POST(request) {
  try {
    ...
    try {
      ...
      vscode["kilocode.customProvider"] = { name: "NovaRoute", baseURL: normalizedBaseUrl, apiKey };
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ success: true, message: "NovaRoute settings removed from Kilo Code" });
  } catch (error) {
