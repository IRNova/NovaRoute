...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-NovaRoute.env");
};
...
const hasNovaRouteConfig = (config) => {
  ...
  if (providers["NovaRoute"]) return true;
...
export async function GET() {
  ...
  const hasNovaRoute = hasNovaRouteConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_NovaRoute_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile NovaRoute",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["NovaRoute"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_NovaRoute_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "NovaRoute configuration removed from jcode",
    });
  } catch (error) {
