...
// Check if settings has NovaRoute config
const hasNovaRouteConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["NovaRoute"];
};

// Read per-agent models.json and return current model id (without "NovaRoute/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["NovaRoute"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      hasNovaRoute: hasNovaRouteConfig(settings),
...
// POST - Update NovaRoute settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `NovaRoute/${model}`;

    // Remove all old NovaRoute/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("NovaRoute/"))
    ...
    // Add fresh NovaRoute models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`NovaRoute/${m}`] = {};
    });

    // Remove old NovaRoute model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("NovaRoute/")) {
    ...
    // Update models.providers.novaroute with all models
    settings.models.providers["NovaRoute"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `NovaRoute/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove NovaRoute settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove NovaRoute from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["NovaRoute"];
    ...
    // Remove NovaRoute models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("NovaRoute/"));
    ...
    // Reset agents.defaults.model.primary if it uses NovaRoute
    if (settings.agents?.defaults?.model?.primary?.startsWith("NovaRoute/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "NovaRoute settings removed successfully",
    });
  } catch (error) {
