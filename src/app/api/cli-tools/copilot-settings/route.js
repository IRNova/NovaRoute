...
const hasNovaRouteConfig = (config) => {
  ...
  return config.some((entry) => entry.name === "NovaRoute");
};

const getNovaRouteEntry = (config) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => entry.name === "NovaRoute") || null;
};
...
export async function GET() {
  try {
    ...
    const entry = getNovaRouteEntry(config);
...
// POST - Apply NovaRoute config to chatLanguageModels.json
export async function POST(request) {
  try {
    ...
    // Replace existing NovaRoute entry or append
    const idx = config.findIndex((e) => e.name === "NovaRoute");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove NovaRoute entry from chatLanguageModels.json
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => e.name !== "NovaRoute");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "NovaRoute removed from Copilot config",
    });
  } catch (error) {
