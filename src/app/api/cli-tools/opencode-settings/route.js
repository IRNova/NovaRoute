...
const hasNovaRouteConfig = (config) => {
  if (!config?.provider) return false;
  return !!config.provider["NovaRoute"];
};
...
export async function GET() {
  try {
    ...
    const providerConfig = config?.provider?.["NovaRoute"];
    const modelMap = providerConfig?.models || {};

    return NextResponse.json({
      installed: true,
      config,
      hasNovaRoute: hasNovaRouteConfig(config),
      configPath: getConfigPath(),
        opencode: {
          models: Object.keys(modelMap),
          activeModel: config?.model?.startsWith("NovaRoute/") ? config.model.replace(/^NovaRoute\//, "") : null,
...
// POST - Apply NovaRoute as openai-compatible provider (multi-model support)
export async function POST(request) {
  try {
    ...
    const keyToUse = apiKey || "sk_NovaRoute";
    const effectiveSubagentModel = subagentModel || modelsArray[0];

    // Ensure provider object
    if (!config.provider) config.provider = {};

    // Preserve any existing NovaRoute provider entry and its models
    const existingProvider = config.provider["NovaRoute"] || { npm: "@ai-sdk/openai-compatible", options: {}, models: {} };
    ...
    config.provider["NovaRoute"] = existingProvider;
    ...
    if (activeModel === "") {
      ...
      if (finalActive) {
        config.model = `NovaRoute/${finalActive}`;
      }
    }
    ...
    config.agent.explorer = {
      ...
      mode: "subagent",
      model: `NovaRoute/${effectiveSubagentModel}`,
    };
...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("NovaRoute/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove NovaRoute provider or specific models from config
export async function DELETE(request) {
  try {
    ...
    if (modelToRemove && config.provider?.["NovaRoute"]?.models) {
      delete config.provider["NovaRoute"].models[modelToRemove];
      
      // If no models left, remove the provider
      if (Object.keys(config.provider["NovaRoute"].models).length === 0) {
        delete config.provider["NovaRoute"];
        if (config.model?.startsWith("NovaRoute/")) delete config.model;
      } else if (config.model === `NovaRoute/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["NovaRoute"].models);
        config.model = `NovaRoute/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire NovaRoute provider
      if (config.provider) delete config.provider["NovaRoute"];
      if (config.model?.startsWith("NovaRoute/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("NovaRoute/")) {
    ...
    return NextResponse.json({
      success: true,
      message: modelToRemove ? `Model "${modelToRemove}" removed` : "NovaRoute settings removed from OpenCode",
    });
  } catch (error) {
