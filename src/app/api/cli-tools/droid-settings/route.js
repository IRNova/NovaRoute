...
// Check if settings has NovaRoute customModels
const hasNovaRouteConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:NovaRoute"));
};
...
// POST - Update NovaRoute customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing NovaRoute configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:NovaRoute"));
...
// DELETE - Remove NovaRoute customModels only (keep other settings)
