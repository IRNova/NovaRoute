...
// Every validation probe gets a hard timeout so a silent/hung endpoint can't
// stall the modal. Callers may override via options.signal.
const VALIDATE_TIMEOUT_MS = 15000;
const fetchT = (url, options = {}) => fetch(url, { signal: AbortSignal.timeout(VALIDATE_TIMEOUT_MS), ...options });

function describeValidateError(err) {
  if (err?.name === "TimeoutError" || /aborted due to timeout/i.test(err?.message || "")) {
    return `Timeout after ${VALIDATE_TIMEOUT_MS / 1000}s — provider unreachable from this server (network, firewall or geo-block?)`;
  }
...
async function probeWebProvider(provider, apiKey) {
  ...
  const res = await fetchT(url, { method: cfg.method, headers, body, signal: AbortSignal.timeout(8000) });
...
async function probeMediaProvider(provider, apiKey) {
  ...
  const res = await fetchT(cfg.baseUrl, {
    method,
    headers,
  ...
  try {
    ...
    try {
      if (isOpenAICompatibleProvider(provider)) {
        ...
        const res = await fetchT(modelsUrl, {
      ...
      if (isCustomEmbeddingProvider(provider)) {
        ...
        const modelsRes = await fetchT(`${baseUrl}/models`, {
          headers: { "Authorization": `Bearer ${apiKey}` },
        });
        if (modelsRes.ok) {
        ...
        const embedRes = await fetchT(`${baseUrl}/embeddings`, {
          method: "POST",
      ...
      if (isAnthropicCompatibleProvider(provider)) {
        ...
        const res = await fetchT(messagesUrl, {
          method: "POST",
          headers: {
            "x-api-key": apiKey,
      ...
      if (provider === "azure") {
        ...
        const azureRes = await fetchT(url, {
          method: "POST",
          headers,
          body: JSON.stringify({
            messages: [{ role: "user", content: "test" }],
      ...
      switch (provider) {
        case "openai":
          const openaiRes = await fetchT("https://api.openai.com/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "vercel-ai-gateway":
          const vercelAiGatewayRes = await fetchT("https://ai-gateway.vercel.sh/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "gemini":
          const geminiRes = await fetchT(`https://generativelanguage.googleapis.com/v1/models?key=${apiKey}`);
          isValid = geminiRes.ok;
          break;

        case "openrouter":
          const openrouterRes = await fetchT("https://openrouter.ai/api/v1/models", {
            headers: { "Authorization": `Bearer ${apiKey}` },
          });
        ...
        case "volcengine-ark":
        case "byteplus": {
          const res = await fetchT(PROVIDERS[provider]?.baseUrl, {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${apiKey}`,
              "content-type": "application/json",
        ...
        case "nvidia": {
          ...
          const res = await fetchT(endpoints[provider], { headers, signal: AbortSignal.timeout(8000) });
        ...
        case "opencode-go": {
          const res = await fetchT("https://opencode.ai/zen/go/v1/chat/completions", {
            method: "POST",
        ...
        case "deepgram": {
          const res = await fetchT("https://api.deepgram.com/v1/projects", {
        ...
        case "blackbox": {
          const res = await fetchT("https://api.blackbox.ai/chat/completions", {
            method: "POST",
            headers: {
              "Authorization": `Bearer ${apiKey}`,
              "Content-Type": "application/json",
        ...
        case "grok-web": {
          // Full multi-cookie passthrough with newline/typo normalization.
          const normalized = String(apiKey || "")
            .replace(/\r?\n+/g, "")
            .split(";")
            .map((seg) => seg.trim())
            .filter(Boolean)
            .map((seg) => seg.replace(/^sso[-_]rw=/i, "sso_rw="))
            .join("; ");
          const cookieHeader = normalized.includes("=")
            ? normalized
            : `sso=${normalized}`;
          ...
          const res = await fetchT("https://grok.com/rest/app-chat/conversations/new", {
            method: "POST",
            headers: {
              Accept: "*/*",
          ...
          if (res.status === 401 || res.status === 403) {
            isValid = false;
            // Distinguish a Cloudflare IP challenge (cookie may be fine) from a
            // genuinely rejected cookie — both arrive as 403.
            const bodyText = await res.text().catch(() => "");
            const cfChallenge = res.headers.get("server")?.toLowerCase().includes("cloudflare")
              && /just a moment|cf-chl|challenge-platform|_cf_chl|enable javascript/i.test(bodyText);
            if (res.status === 403 && cfChallenge) {
              error = "Blocked by Cloudflare IP challenge — grok.com is challenging this server's datacenter IP. The cookie itself may be valid; try from a residential IP or via proxy.";
            } else if (res.status === 403) {
              error = "Forbidden — SSO cookie expired/invalid, or your account lacks access to this endpoint";
            } else {
        ...
        case "perplexity-web": {
          ...
          const res = await fetchT("https://www.perplexity.ai/rest/sse/perplexity_ask", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        case "zai-web": {
          // Token proof via the live models endpoint — works without captcha.
          try {
            const { fetchZaiLiveModels } = await import("open-sse/executors/zai-web.js");
            const models = await fetchZaiLiveModels(apiKey);
            if (models && models.length > 0) {
              isValid = true;
            } else {
              isValid = false;
              error = "Z.ai rejected the token (no live models returned)";
            }
          } catch (err) {
            isValid = false;
            error = `Z.ai check failed: ${err.message}`;
          }
          break;
        }
        ...
        default: {
          ...
          const cfg = PROVIDERS[provider];
          if (!cfg || !cfg.baseUrl || (cfg.format !== "openai" && cfg.format !== "claude")) {
          ...
          if (cfg.authType === "cookie") {
            // Cookie providers: paste full "a=1; b=2" header OR bare token wrapped
            // in the provider's declared cookie name.
            const raw = String(apiKey || "").trim();
            if (raw.includes("=")) headers["Cookie"] = raw;
            else headers["Cookie"] = `${cfg.cookieName || "token"}=${raw}`;
          } else if (cfg.authHeader === "x-api-key") {
            headers["X-API-Key"] = apiKey;
          } else {
            headers["Authorization"] = `Bearer ${apiKey}`;
          }
          // Anthropic Messages endpoints: no /models sibling — probe with a
          // minimal Messages-shaped body instead.
          if (cfg.format === "claude") {
            headers["anthropic-version"] = "2023-06-01";
            const claudeRes = await fetchT(cfg.baseUrl, {
              method: "POST",
              headers,
              body: JSON.stringify({
            ...
            if (!isValid) error = `Upstream rejected the credential (HTTP ${claudeRes.status})`;
            break;
          }
          ...
          let probeOk = null;
          try {
            const probeRes = await fetchT(modelsUrl, { headers, signal: AbortSignal.timeout(8000) });
          ...
          const chatRes = await fetchT(cfg.baseUrl, {
            method: "POST",
            headers,
      ...
      isValid = false;
      error = describeValidateError(err);
    }
