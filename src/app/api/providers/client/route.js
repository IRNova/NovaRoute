...
export async function GET(request) {
  try {
    ...
    const testStatusFilteredConnections = accountFilteredConnections.filter((conn) => {
      if (testStatus === "all") return true;
      const status = conn.testStatus || null;
      const isOk = status === "active" || status === "success";
    ...
    const searchFilteredConnections = search
      ? testStatusFilteredConnections.filter((conn) => {
          const haystack = [
            conn.name,
            conn.email,
            conn.displayName,
