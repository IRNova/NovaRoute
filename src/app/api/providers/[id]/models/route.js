...
import { refreshGoogleToken, refreshCodexToken, refreshCopilotToken, updateProviderCredentials } from "@/sse/services/tokenRefresh";
...
import { refreshXaiToken } from "open-sse/services/tokenRefresh.js";
...
const PROVIDER_MODELS_CONFIG = {
  ...
  github: {
    customResolver: async (connection) => {
      const accessToken = connection.accessToken || connection.providerSpecificData?.accessToken;
      if (!accessToken) return { models: [], warning: "No GitHub access token found" };
      ...
      const parseFn = (data) => {
        ...
        return data.data
          ...
          .map(m => ({ id: m.id, name: m.name || m.id, version: m.version, capabilities: m.capabilities, isDefault: m.model_picker_enabled === true }));
      };
      let copilotToken = connection.providerSpecificData?.copilotToken;
      let copilotExpiresAt = connection.providerSpecificData?.copilotTokenExpiresAt ? connection.providerSpecificData.copilotTokenExpiresAt * 1000 : 0;
      const now = Date.now();
      const remaining = copilotExpiresAt - now;
      if (!copilotToken || remaining < 300000) {
        const refreshed = await refreshCopilotToken(accessToken);
        if (refreshed?.token) {
          copilotToken = refreshed.token;
          copilotExpiresAt = refreshed.expiresAt ? refreshed.expiresAt * 1000 : Date.now() + 1800000;
          await updateProviderCredentials(connection.id, {
      ...
      if (!copilotToken) return { models: [], warning: "Could not obtain Copilot token" };
      try {
        const res = await fetch(COPILOT_MODELS_URL, { headers: { ...COPILOT_HEADERS, Authorization: `Bearer ${copilotToken}` } });
        if (res.ok) {
      ...
      return { models: [], warning: "Failed to fetch Copilot models" };
    }
  },
  ...
  xai: {
    customResolver: async (connection) => {
      ...
      if (!token) return { models: [], warning: "No valid token found" };
      const fetchModels = async (accessToken) => {
      ...
      if (!res.ok && (res.status === 401 || res.status === 403) && refreshToken) {
        try {
          ...
          if (refreshed?.access_token) {
            ...
            res = await fetchModels(refreshed.access_token);
          }
        } catch {}
      }
      if (res.ok) {
        const data = await res.json();
        const models = (data.data || []).map(m => ({ id: m.id, name: m.id, contextLength: m.context_length }));
        if (models.length > 0) return { models };
      }
      return { models: [], warning: "Failed to fetch xAI models" };
    }
  },
  ...
  cohere: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    parseResponse: (data) => (data.models || []).map((m) => ({ id: m.id, name: m.name || m.id })),
  },
  ...
  ollama: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    parseResponse: (data) => (data.models || []).map((m) => ({ id: m.name, name: m.name, details: m.details })),
  },
  ...
  freeaiapikey: createOpenAIModelsConfig("https://freeaiapikey.com/v1/models"),
  ...
  "zai-web": {
    customResolver: async (connection) => {
      try {
        ...
        const token = connection.accessToken || connection.apiKey || "";
        const models = await fetchZaiLiveModels(token);
        if (models && models.length > 0) return { models };
        return {
          models: [],
          warning: "Live model extraction failed — showing nothing instead of a fake catalog",
        };
      } catch (err) {
        return { models: [], warning: `Z.ai extraction failed: ${err.message}` };
      }
    },
  },
  "gemini-cli": {
...
export async function GET(request, { params }) {
  try {
    ...
    if (!config && PROVIDERS[connection.provider]?.modelsFetcher?.url) {
      const fetcher = PROVIDERS[connection.provider].modelsFetcher;
      const token = connection.accessToken || connection.apiKey;
      const headers = { "Content-Type": "application/json" };
      if (token && connection.authType !== "none") headers.Authorization = `Bearer ${token}`;
      const response = await fetch(fetcher.url, { method: "GET", headers, cache: "no-store" });
      if (response.ok) {
        const data = await response.json();
        const models = parseOpenAIStyleModels(data)
          .map((m) => ({
            ...
            contextLength: m?.context_length || m?.contextLength || m?.context_window || m?.limit?.context,
            maxOutputTokens: m?.max_output_tokens || m?.maxOutputTokens || m?.limit?.output,
          }))
      ...
      return NextResponse.json({
        ...
        models: [],
        source: "live_only",
        warning: "Live models endpoint unavailable — using the built-in catalog",
        });
    }
    if (!config) {
      // Claude-format registry providers (agentrouter, glm, kimi, ...) expose a
      // /models list with Anthropic auth. agentrouter's WAF requires the full
      // Claude CLI wire image or it rejects with 401.
      const cfg = PROVIDERS[connection.provider];
      if (cfg?.format === "claude" && cfg?.baseUrl) {
        const base = cfg.baseUrl.replace(/\/messages$/, "").replace(/\/$/, "");
        const headers = {
          "Content-Type": "application/json",
          "x-api-key": connection.apiKey || "",
          "anthropic-version": ANTHROPIC_API_VERSION,
          ...(connection.provider === "agentrouter" ? CLAUDE_CLI_SPOOF_HEADERS : {}),
        };
      ...
      if (staticModels.length > 0) {
        return NextResponse.json({
          ...
          warning: "Provider has no live models endpoint; using the built-in catalog",
        });
      }
    ...
    if (typeof config.customResolver === "function") {
      ...
      // HONESTY GATE: resolvers that fell back to their built-in static catalog
      // must NOT present fabricated model lists as real. Return empty + reason.
      if (result && !result.error && Array.isArray(result.models) && /static catalog/i.test(result.warning || "")) {
        return NextResponse.json({
          provider: connection.provider,
          connectionId: connection.id,
          models: [],
