...
async function getInternalHeaders() {
  ...
  try {
    ...
    // Prefer an unrestricted key so client-key model/group policy can't make
    // every self-test fail (handler-level CLI-token bypass also covers this).
    const unrestricted = active.find((k) =>
      k.modelAccessMode !== "restricted" && !(Array.isArray(k.allowedModels) && k.allowedModels.length > 0)
    );
    apiKey = unrestricted?.key || active[0]?.key || null;
  } catch {}
