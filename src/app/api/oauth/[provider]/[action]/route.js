...
import { resolveOAuthProviderId } from "@/shared/constants/oauthProviderAliases.js";
...
export async function GET(request, { params }) {
  try {
    const { provider: rawProvider, action } = await params;
    // Normalize registry card ids (agy, xai-oauth, …) to the canonical flow id so
    // proxy/session special-cases and the saved connection all use one name.
    const provider = resolveOAuthProviderId(rawProvider);
...
export async function POST(request, { params }) {
  try {
    const { provider: rawProvider, action } = await params;
    // Same normalization as GET — see comment there.
    const provider = resolveOAuthProviderId(rawProvider);
    let body;
    try {
    ...
    if (action === "exchange") {
      ...
      // Save to database under the CARD id the user connected from (rawProvider),
      // not the canonical flow id — dashboard cards, /v1/models namespaces and
      // credential lookups all key off the registry card id. For non-aliased
      // providers rawProvider === provider, so this is a no-op for them.
      const connection = await createProviderConnection({
    ...
    if (action === "poll") {
      ...
      if (result.success) {
        // Save to database under the card id (see the exchange path comment).
        const connection = await createProviderConnection({
          provider: rawProvider,
          authType: "oauth",
          ...result.tokens,
