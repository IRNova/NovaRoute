 ...
 */
export default function OAuthModal({ isOpen, provider: rawProvider, providerInfo, onSuccess, onClose, oauthMeta, idcConfig }) {
  // Normalize registry card ids (agy → antigravity, …) so flow decisions below
  // (proxy providers, device-code list, fetch URLs) match the canonical id the
  // API route also normalizes to.
            ...
            {authMode === "paste-token" && (
              <div className="space-y-3">
                {ideStatus && !ideStatus.installed && (
                  <div className={`px-3 py-2 rounded-lg text-sm ${PASTE_TOKEN_PROVIDERS[provider].ideOptional ? "bg-info/10 text-info" : "bg-warning/10 text-warning"}`}>
