...
// Providers that need no auth — always show in model selector
  // Free/no-auth providers — but hidden ones (dead services, unwired dupes)
...
export default function ModelSelectModal({
  ...
  const activeConnectionIds = useMemo(
    () => activeProviders
      .filter((provider) => provider?.id && provider?.provider)
      .map((provider) => provider.id),
    [activeProviders],
  );

  useEffect(() => {
    if (!isOpen || activeConnectionIds.length === 0) return undefined;

    let cancelled = false;
    Promise.all(activeProviders
      .filter((provider) => provider?.id && provider?.provider)
          ...
          const models = Array.isArray(data.models)
            ...
            : Array.isArray(data.data)
              ...
              : Array.isArray(data.results)
                ? data.results
                : [];
          return [connection.id, models];
        } catch {
          return [connection.id, []];
        }
      }))
      .then((entries) => {
        if (!cancelled) setLiveProviderModels(Object.fromEntries(entries));
      })
      .catch((error) => {
  ...
  }, [isOpen, activeConnectionIds, activeProviders]);
  ...
  const fetchModelTestResults = async () => {
    const providerIds = new Set([
      ...activeProviders.map((provider) => provider.provider).filter(Boolean),
    ...
    try {
      const entries = await Promise.all([...providerIds].map(async (providerId) => {
        ...
        const data = await res.json();
        return [alias, data.results || {}];
      }));
      setModelTestResults(Object.fromEntries(entries));
    } catch (error) {
  ...
  useEffect(() => {
    if (isOpen) fetchModelTestResults();
  ...
  const groupedModels = useMemo(() => {
    ...
    const withTestStatus = (model, alias) => {
      const result = modelTestResults[alias]?.[model.id];
      if (!result?.status) return model;
      return { ...model, testStatus: result.status, testedAt: result.testedAt, testError: result.error };
    };

    const activeTestedModels = (alias) => Object.entries(modelTestResults[alias] || {})
      .filter(([, result]) => result?.status === "ok")
      .map(([id, result]) => withTestStatus({ id, name: id, value: `${alias}/${id}`, isTested: true }, alias));

    const rawLiveId = (model) => (typeof model === "string" ? model : model?.id || model?.name || model?.model || "");
    const liveModelsForProvider = (providerId, outputAlias) => activeProviders
      .filter((connection) => connection.provider === providerId && connection.id)
      .flatMap((connection) => (liveProviderModels[connection.id] || []).map((model) => {
        const rawId = rawLiveId(model);
        if (!rawId) return null;
        const id = rawId.includes("/") ? rawId.split("/").pop() : rawId;
        const value = rawId.includes("/") ? rawId : `${outputAlias}/${rawId}`;
        const name = typeof model === "string" ? rawId : model?.name || model?.displayName || rawId;
        return withTestStatus({ id, name, value, isLive: true }, outputAlias);
      }).filter(Boolean));
    ...
    sortedProviderIds.forEach((providerId) => {
      ...
      if (providerInfo.passthroughModels) {
        ...
        if (kindFilter && TYPED_KINDS.has(kindFilter)) {
          ...
          // LLM/null kind: merge live provider catalog (same source as chat), hardcoded models, and user-added models
          const registeredLlms = customRegisteredModels.filter((m) => !getModelKind(m) || getModelKind(m) === "llm");
          const testedModels = activeTestedModels(alias);
          const liveModels = liveModelsForProvider(providerId, alias);
          const seen = new Set([...liveModels, ...aliasModels, ...registeredLlms, ...testedModels].map((m) => m.value));
          ...
          combined = [
            ...liveModels,
            ...testedModels.filter((m) => !liveModels.some((live) => live.value === m.value)),
            ...registeredLlms.filter((m) => !liveModels.some((live) => live.value === m.value) && !testedModels.some((tested) => tested.value === m.value)),
            ...aliasModels.filter((m) => !liveModels.some((live) => live.value === m.value) && !registeredLlms.some((registered) => registered.value === m.value) && !testedModels.some((tested) => tested.value === m.value)),
            ...hardcoded.map((m) => withTestStatus(m, alias)),
          ];
        }
        ...
        const testedModels = activeTestedModels(providerId).map((m) => ({
          ...m,
          value: `${nodePrefix}/${m.id}`,
        }));
        const liveModels = liveModelsForProvider(providerId, nodePrefix).map((m) => withTestStatus(m, providerId));
        const seen = new Set(liveModels.map((m) => m.value));
        const mergedModels = [
          ...liveModels,
          ...testedModels.filter((m) => !seen.has(m.value)),
          ...nodeModels.map((m) => withTestStatus(m, providerId)).filter((m) => !seen.has(m.value)),
          ...registeredCustom.map((m) => withTestStatus(m, providerId)).filter((m) => !seen.has(m.value)),
        ];
        ...
        const customRegisteredModels = customModels
          ...
          .map((m) => withTestStatus({ id: m.id, name: m.name || m.id, value: `${alias}/${m.id}`, isCustom: true }, alias));

        const testedModels = activeTestedModels(alias).filter((m) => !hardcodedIds.has(m.id));
        const liveModels = liveModelsForProvider(providerId, alias);
        const merged = [
          ...liveModels,
          ...hardcodedModels.map((m) => withTestStatus({ id: m.id, name: m.name, value: `${alias}/${m.id}`, kind: getModelKind(m) }, alias)),
          ...testedModels,
          ...customAliasModels.map((m) => withTestStatus(m, alias)),
    ...
    return groups;
  }, [filteredActiveProviders, modelAliases, allProviders, providerNodes, customModels, disabledModels, modelTestResults, liveProviderModels, kindFilter, activeProviders, cursorModels]);
                ...
                return (
                  ...
                  >
                    <span className="flex items-center gap-1">
                      ...
                      {model.testStatus === "ok" && !isPlaceholder && (
                        <span className="text-[9px] font-semibold text-emerald-600 dark:text-emerald-400">active</span>
                      )}
                      {model.testStatus === "error" && !isPlaceholder && (
                        <span className="text-[9px] font-semibold text-red-600 dark:text-red-400">broken</span>
                      )}
                      {isPlaceholder ? (
                        <>
