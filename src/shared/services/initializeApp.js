...
async function runHeavyStartup() {
  ...
  // Daily SQLite safety backups (keeps 7). In-app scheduler — no cron needed.
  import("@/lib/db/backupScheduler.js")
    .then(({ startDailyDbBackup }) => startDailyDbBackup())
    .catch((e) => console.log("[DbBackup] scheduler start failed:", e.message));

  // Periodic provider health checks keep testStatus/badges fresh.
  import("@/lib/providersHealthCheck.js")
    .then(({ startProviderHealthCheck }) => startProviderHealthCheck())
    .catch((e) => console.log("[ProvidersHealthCheck] scheduler start failed:", e.message));

  // Periodic proxy-pool tests auto-disable dead nodes.
  import("@/lib/proxyPoolsHealthCheck.js")
    .then(({ startProxyPoolsHealthCheck }) => startProxyPoolsHealthCheck())
    .catch((e) => console.log("[PoolsHealthCheck] scheduler start failed:", e.message));

  // One-time at-rest encryption of stored provider credentials.
  import("@/lib/db/repos/connectionsRepo.js")
    .then(({ migrateEncryptConnections }) => migrateEncryptConnections())
    .then((r) => {
      if (r?.skipped) console.log("[Crypto] credential encryption skipped (no API_KEY_SECRET)");
    })
    .catch((e) => console.log("[Crypto] migration failed:", e.message));

  // Telegram bot bridge (userbot draft approval buttons). The poller is
  // idempotent and self-terminates when the bot config disables or leaves
  // polling mode — without this the buttons die after every restart.
  import("@/lib/nova/telegram.js")
    .then(({ ensureTelegramPoller }) => ensureTelegramPoller())
    .catch((e) => console.log("[InitApp] telegram poller start failed:", e.message));

  // Discord channel adapter (Gateway WebSocket). Idempotent; no-ops when the
  // channel config is disabled or has no token.
  import("@/lib/nova/discord.js")
    .then(({ ensureDiscordBot }) => ensureDiscordBot())
    .catch((e) => console.log("[InitApp] discord bot start failed:", e.message));
}
...
async function autoStartMitm(settings) {
  ...
  try {
    ...
    await startMitm(activeKey?.key || "sk_NovaRoute", password);
