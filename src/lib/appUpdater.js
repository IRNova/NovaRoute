...
// Collect PIDs of all NovaRoute-related processes (excluding current)
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        // Match anything running from NovaRoute install dir or wrapper cli.js
        const isAppProcess = lower.includes("novaroute") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("novaroute") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "novaroute");
  }
  return path.join(os.homedir(), ".novaroute");
}
...
// Resolve npx/NovaRoute binary to relaunch after update (cross-platform)
function resolveRelaunchCommand() {
  const isWin = process.platform === "win32";
  // Prefer `npx novaroute` — works regardless of global bin path changes after npm i -g
...
export function spawnUpdaterAndExit(packageName = UPDATER_CONFIG.npmPackageName) {
  // Fail closed. This installs an npm package and relaunches the server into
  // it, so it must never run against a package we do not control. See
  // UPDATER_CONFIG for how to enable the channel.
  if (!UPDATER_CONFIG.enabled || !packageName) {
    throw new Error(
      "Self-update is disabled: no IRNova-owned npm package is configured. " +
        "Set NOVAROUTE_UPDATE_CHANNEL=npm and NOVAROUTE_NPM_PACKAGE to enable it."
    );
  }
