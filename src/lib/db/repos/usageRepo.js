...
export async function saveRequestUsage(entry) {
  try {
    ...
    // Every surface funnels through here with provider, model, tokens and cost
    // resolved, so this is where the trace gets its GenAI attributes.
    try {
      const { recordUsageSpan } = await import("@/lib/monitoring/otel.js");
      recordUsageSpan({
    ...
    } catch { /* tracing must never affect accounting */ }
...
/**
 * Total cost recorded so far in the current calendar month (from usageDaily).
 * Used by the Costs dashboard's monthly budget progress.
 */
export async function getMonthCost(now = new Date()) {
  try {
    const db = await getAdapter();
    const startKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-01`;
    const rows = db.all(`SELECT data FROM usageDaily WHERE dateKey >= ?`, [startKey]);
    let cost = 0;
    let requests = 0;
    for (const r of rows) {
