...
} from "@/lib/security/fieldCipher.js";
...
function rowToConn(row) {
  if (!row) return null;
  const extra = decryptSensitiveFields(parseJson(row.data, {}));
  return {
    ...extra,
    id: row.id,
...
function connToRow(c) {
  ...
  return {
    ...
    data: stringifyJson(encryptSensitiveFields(rest)),
    createdAt,
    updatedAt,
  };
}
...
export async function createProviderConnection(data) {
  ...
  db.transaction(() => {
    ...
    if (data.authType === "oauth" && data.email) {
      ...
      // No-auth/free providers are singleton connections. The setup endpoint may
      // run after every app update/reinstall; update the existing row instead
      // of creating duplicate CLI/free-provider entries each time.
      existing = all.find(c => c.authType === "none");
    }
...
// One-time lazy migration: re-save every connection so legacy plaintext
// credentials get encrypted on disk (no-op when API_KEY_SECRET is unset).
const ENC_PREFIX = "enc:v1:";
const SENSITIVE = ["apiKey", "accessToken", "refreshToken", "idToken"];
export async function migrateEncryptConnections() {
  if (!process.env.API_KEY_SECRET) return { skipped: true, reason: "no API_KEY_SECRET" };
  try {
    const db = await getAdapter();
    ...
    for (const r of rows) {
      ...
      const hasPlain = SENSITIVE.some(
        (f) => typeof d[f] === "string" && d[f] && !d[f].startsWith(ENC_PREFIX)
      );
      if (hasPlain) {
        await updateProviderConnection(r.id, {});
        migrated += 1;
      }
    }
    if (migrated) console.log(`[Crypto] encrypted credentials on ${migrated} connection(s)`);
    return { migrated };
  } catch (e) {
    console.error("[Crypto] migrateEncryptConnections failed:", e?.message || e);
