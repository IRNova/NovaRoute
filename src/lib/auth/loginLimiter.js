...
export function getClientIp(request) {
  // Trusted: set from TCP socket by custom-server.js — only accepted when the
  // request carries the wrapper's per-boot stamp, so forged headers cannot
  // mint fresh limiter buckets.
  const realIp = request.headers.get("x-9r-real-ip");
  if (realIp && hasTrustedPeerStamp(request)) return realIp;
