...
const previousDefaultRegExp = /^# novaroute-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# novaroute-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via NovaRoute gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# novaroute-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# novaroute-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
