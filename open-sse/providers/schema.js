 ...
 * @property {string}   category      "apikey"|"oauth"|"webCookie"|"cli"|"freeTier"|"free"|"local" drives UI auth grouping. REQUIRED.
 ...
 * @property {Array}   [credentialFields] Dynamic credential input fields for UI.
 *   Each: { id:string, label:string, placeholder?:string, type?:'text'|'password'|'textarea', required?:boolean, hint?:string }.
 *   When defined, the UI renders these fields instead of the single apiKey input.
 *   Values are stored in connection data.credentialData (JSON object keyed by field id).
 *   The primary apiKey is set to the first required field's value for backward compat.
