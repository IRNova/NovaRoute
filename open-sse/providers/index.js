...
for (const entry of REGISTRY) {
  ...
  if (entry.models !== undefined) {
    const modelKey = entry.alias || entry.id;
    if (PROVIDER_MODELS[modelKey] !== undefined) {
      console.warn(`[registry] duplicate models key "${modelKey}" from "${entry.id}" overwrites an earlier provider's list`);
    }
    PROVIDER_MODELS[modelKey] = entry.models.map(normalizeModel);
  }
