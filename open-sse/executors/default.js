...
export class DefaultExecutor extends BaseExecutor {
  ...
  buildUrl(model, stream, urlIndex = 0, credentials = null) {
    ...
    // Generic {fieldName} substitution from credentialData / credentials — lets
    // registries declare URLs that embed per-connection values (org ids etc.).
    if (url && url.includes("{")) {
      const cd = credentials?.providerSpecificData?.credentialData || {};
      const resolved = url.replace(/\{([A-Za-z0-9_]+)\}/g, (m, key) => {
        const val = cd[key] ?? credentials?.[key];
        return val !== undefined && val !== null && String(val).trim() !== "" ? encodeURIComponent(String(val).trim()) : m;
      });
  ...
  buildHeaders(credentials, stream = true, url, model) {
    ...
    // Cookie-auth providers (webCookie category): build a real Cookie header from
    // either a pasted full cookie string ("a=1; b=2") or a bare token wrapped in
    // the provider's declared cookie name, plus any extra named fields from
    // providerSpecificData.credentialData (e.g. cf_clearance, sso_rw, formkey).
    if (this.config.authType === "cookie") {
      ...
      const apiKeyStr = String(credentials.apiKey || "").trim();
      if (apiKeyStr.includes(";")) {
        // Full cookie header pasted by the user — take it verbatim.
        apiKeyStr.split(";").forEach(pushPair);
      } else if (apiKeyStr.includes("=")) {
        pushPair(apiKeyStr);
      } else if (apiKeyStr) {
        pushPair(`${this.config.cookieName || "token"}=${apiKeyStr}`);
      }
      const cd = credentials.providerSpecificData?.credentialData;
      if (cd && typeof cd === "object") {
        for (const [k, v] of Object.entries(cd)) {
          if (v === undefined || v === null || String(v).trim() === "") continue;
          pushPair(String(v).includes("=") ? v : `${k}=${String(v).trim()}`);
        }
      }
      if (parts.length > 0) {
      ...
      // Providers needing bespoke headers sourced from credential fields
      // (e.g. Poe-Formkey from the "formkey" field).
      const credHeaderMap = this.config.credentialHeaders;
      if (credHeaderMap && typeof credHeaderMap === "object") {
        const cd2 = credentials.providerSpecificData?.credentialData || {};
        for (const [headerName, sourceField] of Object.entries(credHeaderMap)) {
          const val = cd2[sourceField] ?? credentials[sourceField];
          if (val !== undefined && val !== null && String(val).trim() !== "") {
            headers[headerName] = String(val).trim();
          }
        }
      }
    }
