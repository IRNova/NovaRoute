...
function createRequestContextResponse() {
  // AgentService asks every run for client context. NovaRoute has no IDE file
      ...
      try {
        while (!finished) {
          ...
          pending = decodeAgentFrames(pending, (payload) => {
            ...
            // Return an empty context; NovaRoute is not coupled to an editor.
            if (serverMessage.has(2)) {
              ...
              if (execRequest.has(10)) {
                ...
                // (shell, read, write, …) that NovaRoute cannot service. Fail the
