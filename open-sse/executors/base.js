...
export class BaseExecutor {
  ...
  buildHeaders(credentials, stream = true) {
    ...
    if (this.provider?.startsWith?.("anthropic-compatible-")) {
      ...
      // Standard auth for other providers. Most use Authorization: Bearer, but
      // registry transports may declare a different wire image (for example
      // AgentRouter requires Claude-style x-api-key; tests used that header but
      // runtime previously ignored it, causing upstream content-blocked 400s).
      const auth = this.config?.auth;
      const token = credentials.accessToken || credentials.apiKey;
      if (auth?.header && token && auth.header.toLowerCase() !== "authorization") {
