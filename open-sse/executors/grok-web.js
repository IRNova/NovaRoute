...
export class GrokWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }) {
    ...
    // Accept EITHER the bare sso value OR a full multi-cookie string
    // ("sso=…; sso_rw=…; cf_clearance=…" possibly pasted across multiple lines).
    // Robust parse: strip newlines, split on ';', fix common name typos,
    // then rebuild a clean single-line Cookie header.
    if (credentials.apiKey) {
      const raw = String(credentials.apiKey).replace(/\r?\n+/g, "").trim();
      const segments = raw
        .split(";")
        .map((seg) => seg.trim())
        .filter(Boolean)
      ...
      const isPairList = segments.length > 1 || /^[A-Za-z0-9_-]+=/.test(segments[0] || "");
      if (isPairList && !/^sso=$/.test(segments[0])) {
        // Multi-cookie session (or explicit pair) — send verbatim as the header.
        headers["Cookie"] = segments.join("; ");
      } else {
        const bare = segments[0] || "";
        headers["Cookie"] = `sso=${bare.startsWith("sso=") ? bare.slice(4).trim() : bare}`;
      }
    }
