...
export class PerplexityWebExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log }) {
    ...
    if (credentials.accessToken) {
      ...
      // Accept bare session-token OR a full cookie header (may carry extra items).
      headers["Cookie"] =
        rawCookie.includes(";") || rawCookie.startsWith("__Secure-next-auth.session-token=")
          ? rawCookie
          : `__Secure-next-auth.session-token=${rawCookie}`;
    }
