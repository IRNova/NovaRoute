...
import { orderComboModels, recordComboTargetResult, resetComboOrderState } from "./comboStrategies.js";
...
export function resetComboRotation(comboName) {
  ...
  // Also clear the strategy engine's rotation/usage state so a re-configured
  // combo starts fresh.
  resetComboOrderState(comboName);
}
 ...
 * @param {string} [options.comboStrategy] - Strategy id (fallback, round-robin, weighted, auto, ...)
 * @param {number|string} [options.comboStickyLimit=1] - Requests per combo model before switching
 * @param {Object} [options.comboStrategyConfig] - Full per-combo strategy config (settings.comboStrategies[comboName]) with weights/tuning
 * @param {string} [options.taskType] - Detected task type (for stats-aware strategies)
 * @param {boolean} [options.autoSwitch=true] - Float capability-fit models to the front
 * @returns {Promise<Response>}
 */
export async function handleComboChat({ body, models, handleSingleModel, log, comboName, comboStrategy, comboStickyLimit = 1, autoSwitch = true, comboStrategyConfig = null, taskType = null }) {
  const strategy = comboStrategy || comboStrategyConfig?.fallbackStrategy || "fallback";
  const strategyConfig = comboStrategyConfig && typeof comboStrategyConfig === "object" ? comboStrategyConfig : {};

  // Apply ordering strategy (fallback/round-robin legacy + data-driven strategies).
  let rotatedModels = await orderComboModels({
    models,
    comboName,
    strategy,
    config: {
      ...strategyConfig,
      stickyLimit: strategyConfig.stickyLimit != null ? strategyConfig.stickyLimit : comboStickyLimit,
    },
    context: { body, taskType },
  });
  ...
  for (let i = 0; i < rotatedModels.length; i++) {
    ...
    try {
      ...
      // Feed telemetry (fail-open) so stats-aware strategies learn over time.
      recordComboTargetResult({ modelStr, ok: result.ok, latencyMs: Date.now() - attemptStarted, taskType }).catch(() => {});
      ...
      recordComboTargetResult({ modelStr, ok: false, latencyMs: Date.now() - attemptStarted, taskType }).catch(() => {});
