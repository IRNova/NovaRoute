...
import { isNoLogApiKey } from "@/lib/db/repos/apiKeysRepo.js";
...
export async function saveUsageStats({ provider, model, tokens, connectionId, apiKey, endpoint, label = "USAGE", silent = false, taskType, latencyMs }) {
  ...
  // Keys with noLog set skip all usage persistence (their spend is not counted).
  if (apiKey) {
    try {
      if (await isNoLogApiKey(apiKey)) return;
    } catch { /* fail-open: never let the check drop logging */ }
  }
  ...
  // Predictive routing feedback: feed the rolling (taskType, provider, model)
  // performance table used by the adaptive scorer. Fail-open — routing stats
  // must never break usage persistence.
  try {
    const { recordRequest } = await import("../../routing/predictor.js");
    const { getPricingForModel, calculateCostFromTokens } = await import("../../providers/pricing.js");
    const cost = calculateCostFromTokens(tokens, getPricingForModel(provider, model));
    await recordRequest({
      taskType,
      provider,
      model,
