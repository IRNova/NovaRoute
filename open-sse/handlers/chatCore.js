...
import { compressSemantically, formatSemanticLog } from "../rtk/semantic.js";
import { optimizeMessages, formatOptimizerLog } from "../promptOptimizer/index.js";
...
import { recordFailure } from "../routing/predictor.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, onRtkEvent, sourceFormatOverride, providerThinking, taskType, semanticCompressEnabled, semanticCompressUrl, semanticCompressApiKey, semanticCompressModel, semanticCompressTimeoutMs, semanticCompressMinChars, optimizerEnabled, optimizerMode, optimizerTimeoutMs, optimizerLlmEndpoint, optimizerLlmApiKey, optimizerLlmModel }) {
  ...
  // GUARDRAILS pre-flight (fail-open, like the rtk hooks). Only runs when the
  // operator enabled enforcement in the dashboard; masks or blocks before any
  // upstream call is made.
  if (Array.isArray(translatedBody.messages) && translatedBody.messages.length > 0) {
    try {
      const { runGatewayGuardrails } = await import("@/lib/guardrails/sharedRegistry.js");
      const gr = await runGatewayGuardrails(translatedBody.messages, { provider, model });
      if (gr?.modifiedMessages) translatedBody.messages = gr.modifiedMessages;
      if (gr?.blocked) {
        const reason = gr.summary || gr.detections?.map((d) => d.type).filter(Boolean).join(", ") || "policy violation";
        console.warn(`[Guardrails] blocked request provider=${provider} model=${model} reason=${reason}`);
        try {
          ...
          await logAudit({
            ...
            provider,
            model,
            details: `Blocked request: ${reason}`,
          });
        } catch {}
        return new Response(
          JSON.stringify({ error: { type: "invalid_request_error", message: `Request blocked by guardrail policy (${reason}).` } }),
          { status: 400, headers: { "Content-Type": "application/json" } }
        );
      }
    } catch {
      // fail-open: never let guardrail errors break traffic
    }
  }
...
// Savings ledger for the Token Saver dashboard (fire-and-forget; stats must
// not break requests).
try {
  if (rtkStats && onRtkEvent) {
    onRtkEvent({
      provider,
      model,
      bytesBefore: Number(rtkStats.bytesBefore) || 0,
      bytesAfter: Number(rtkStats.bytesAfter) || 0,
      hits: Array.isArray(rtkStats.hits) ? rtkStats.hits.length : 0,
    });
  }
} catch { /* ignore */ }

  // Semantic compression: LLM-backed digest of the oldest context blocks (opt-in).
  const semanticStats = await compressSemantically(translatedBody, {
    enabled: tokenSaverEnabled && semanticCompressEnabled,
    minChars: semanticCompressMinChars || 120000,
    summarizerBaseUrl: semanticCompressUrl,
    summarizerApiKey: semanticCompressApiKey,
    summarizerModel: semanticCompressModel,
    timeoutMs: semanticCompressTimeoutMs,
  });
  const semanticLine = formatSemanticLog(semanticStats);
  if (semanticLine) log?.info?.("SEMANTIC", semanticLine);

  // Prompt Optimizer: enhance vague prompts (opt-in, deterministic by default).
  const optimizerStats = await optimizeMessages(translatedBody, {
    enabled: tokenSaverEnabled && optimizerEnabled,
    mode: optimizerMode || "auto",
    timeoutMs: optimizerTimeoutMs,
    llmEndpoint: optimizerLlmEndpoint,
    llmApiKey: optimizerLlmApiKey,
    llmModel: optimizerLlmModel,
  });
  const optimizerLine = formatOptimizerLog(optimizerStats);
  if (optimizerLine) log?.info?.("OPTIMIZER", optimizerLine);
  ...
  try {
    ...
    recordFailure({ taskType, provider, model, latencyMs: Date.now() - requestStartTime }).catch(() => {});
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, pxpipe: pxpipeSummary, reqTag, log, taskType };
