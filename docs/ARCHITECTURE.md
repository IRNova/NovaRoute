...
NovaRoute is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[NovaRoute Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.novaroute/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.novaroute/usage.json`, `~/.novaroute/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.novaroute/db.json`)
- usage stats: `~/.novaroute/usage.json`
- request log lines: `~/.novaroute/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[NovaRoute Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.novaroute` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/NovaRoute && npm run build`
- Build Docker image: `cd /root/dev/NovaRoute && docker build -t novaroute .`
