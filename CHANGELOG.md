...
## Features
- **Nova Bot**: the agent can write to GitHub and Cloudflare, not only read
  them. Create a repository, create a branch, commit a file, open an issue,
  open a pull request; create, update or delete a DNS record, and purge a zone
  cache. Every write waits for admin approval, the same way the terminal tool
  does, and a DNS deletion names the exact record before asking.
- **Apps**: each app card lists what connecting it actually grants the agent,
  so the page cannot promise capability that is not wired.
- **Dashboard**: new "All tools" page. Around 25 working pages had no link
  anywhere in the navigation and could only be opened by typing a URL; they are
  now grouped by purpose with a search box.
- **Providers**: new "No Key Needed" group for the providers that work without
  any credential.
- **Providers**: the connection test can now probe most providers directly from
  their registry entry, instead of only the 80 that had a hand-written check.
- **Settings**: System and Update is now in the settings navigation.
- **Usage**: the provider filter can hide providers you no longer have
  installed, and labels the ones kept for history.
- **Farsi**: 979 new translations. Every `translate()` string in the app now has
  Farsi, and mixed Farsi and Latin text renders in the correct order.

## Fixes
- **Update**: self-update could not complete on any installation. The installer
  removes development dependencies when it finishes, and the updater then tried
  to build without restoring them, so the build always failed. The update also
  refused an authenticated admin whenever the panel was behind a reverse proxy,
  which is every installation with a domain. Both surfaces now share one worker
  that installs, builds, restores the pruned footprint and restarts cleanly,
  and the panel follows progress instead of waiting on a request that ends with
  the restart.
- **Installer**: the one-line install stopped at the first prompt on any machine
  without a terminal, such as CI, cloud-init, or SSH without a pty. It also
  never installed git, although it clones the repository.
- **Providers**: 11 providers were present in the source but missing from the
  registry index, so they were invisible to the dashboard and the gateway. The
  catalogue is 368 providers, up from 357.
- **Providers**: 15 providers that need no credential were shown as requiring
  an API key.
- **Providers**: model context windows fell back to a fixed 200000 for anything
  unrecognised, including text-to-speech voices, and the quota page drew a usage
  percentage against that number. Real limits now come from the provider
  catalogue, and an unknown limit says so.
- **Providers**: the connection test reported most providers as broken purely
  because no check existed for them, and stored that result, which made other
  parts of the panel skip them. It also accepted any 200 response as success,
  including error pages.
- **Providers**: extracted models appeared and then disappeared. The list was
  shown before it was saved, one request per model, and failures were silent.
- **Dashboard**: warnings were rendered in the middle of the home page while the
  notification bell knew nothing about them. They now live in the bell.
- **Usage**: the provider filter listed every provider that had ever served a
  request, including ones deleted long ago.
- **Interface**: the runtime translator rewrote material icon names, so one icon
  rendered as a word in Farsi.
- **Sidebar**: the displayed version was hardcoded and could not change between
  releases. It now follows the actual build.
- **Changelog**: this page showed placeholder entries rather than the release
  notes shipped with the build.

## Security
- General hardening of the update and settings endpoints. Updating is
  recommended.

## Internal
- `package-lock.json` now ships with the repository, so an install or an update
  no longer resolves its own dependency tree from the registry.
- The provider registry index is generated by `scripts/generate-registry-index.mjs`
  and checked in CI, so a provider cannot be added and left unregistered again.
- 100 unit tests, up from 40.
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point NovaRoute at your own
...
- **RTK**: `X-NovaRoute-Token-Saver` header to bypass token savers per request
...
- **NVIDIA**: add new models and capabilities — IRNova
- **ClinePass**: add provider support — sternelee

## Fixes
- **Usage**: dedupe streaming request-details log entries — Qin Li
- **Claude**: drop foreign thinking signatures in passthrough — IRNova
...
- Refine Qwen vision/video + thinking model patterns — IRNova
...
- **Translator**: map mid-conversation system message to user (claude→openai) — IRNova
...
- **OpenCode**: fix Go GLM — IRNova

# v0.5.12 (2026-06-26)

## Features
- Add token-saver dashboard page — IRNova
...
- Stop double-counting streaming usage at source — IRNova
...
- Token-saver: full width card layout — IRNova
...
## Docs
- README: add Indonesian NovaRoute tutorial video (#1709)
